...
- EQ preset selection now takes effect immediately without restarting
  ...
  live parameter update first: it enumerates the running `eq_band_N`
  PipeWire nodes via `pw-dump` and pushes new Freq/Q/Gain values to
  each via `pw-cli set-param`.  The `filter-chain.service` restart is
  only triggered on the first apply (cold path) or if the live update
  fails, preserving the existing fallback behaviour.

## [1.39.33] - 2026-06-07
### Fixed
- Fan tach RPM always None: `ProtectHome=read-only` in the thermal
  systemd unit prevented lgpio from `chmod`-ing its `.lgd-nfy0`
  notification pipe, silently breaking all edge callbacks.  Fixed by
  adding the repo working directory to `ReadWritePaths`.
- sysfs PWM Permission denied on reboot: `ReadWritePaths=/sys/class/pwm`
  used the symlink path which `ProtectSystem=strict` doesn't follow.
  Replaced with the real RP1 device path
